...
**********************************************************************/
ECGP_error link_CompleteAndResend(u16 sendLen)
{
    ECGP_error res;
    u16 len,in,out;
    ...
    if (len > sendLen) {
        ...
        ECGP_tx_fifo.out += len;
        if (ECGP_tx_fifo.out == in) {
            ECGP_tx_fifo.empty = ECGP_TRUE;
            return 0;
        }
        else{
            return link_CompleteAndResend(0);
        }
    }
    //call phy function to send
    res = ECGP_physicalSend(&ECGP_tx_fifo.buf[out], len);
    if( res == ECGP_ENONE ){
        return len;
    }
    return res;
