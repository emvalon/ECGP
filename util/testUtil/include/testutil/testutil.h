...
#include <setjmp.h>
#include <ext/queue.h>
#include <testutil/testutil_config.h>
...
struct ts_suite* tu_suite_register(tu_testsuite_fn_t* ts, const char *name);
void tu_suit_run();
...
struct ts_suite {
    ...
    tu_pre_test_fn_t  *pre_test;
    void *pre_test_arg;
    tu_post_test_fn_t *post_test;
    void *post_test_arg;
};
...
void tu_suite_set_pre_test_cb(struct ts_suite *tsp, tu_pre_test_fn_t *cb, void *cb_arg);
void tu_suite_set_post_test_cb(struct ts_suite *tsp, tu_post_test_fn_t *cb, void *cb_arg);
...
#define TEST_SUITE_REGISTER(suite_name, pre_test, pre_arg, post_test, post_arg)  \
    do {                                                        \
        struct ts_suite *__tsp;                                 \
        __tsp = tu_suite_register(suite_name, #suite_name);     \
        if (__tsp) {                                            \
            tu_suite_set_post_test_cb(__tsp, post_test, post_arg);     \
            tu_suite_set_pre_test_cb(__tsp, pre_test, pre_arg); \
        }                                                       \
        __tsp;                                                  \
    } while (0)
